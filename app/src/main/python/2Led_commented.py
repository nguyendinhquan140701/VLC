...
def xu_ly_y(array2, values_y, row, threshold_code, input_var):
    ...
    print(f"he so sau khi dich bit:{heso[:]}")
    ...
    def twosCom_decBin(dec, digit):
        ...
        if dec>=0:
            ...
            while len(bin1)<digit :
                bin1 = '0'+bin1
                # print(f"value of digit in twosCom_decbin:{digit}")
            return bin1
        else:
            bin1 = -1*dec
            return bin(bin1-pow(2,digit)).split("0b")[1]
    bin1 = twosCom_decBin(value4, 4)
    bin1 = str(bin1)[::-1]
